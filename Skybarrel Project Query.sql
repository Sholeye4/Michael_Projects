...
Select
CASE
WHEN GENDER IS NULL THEN 'X'
WHEN GENDER = '' THEN 'X'
ELSE GENDER END AS [Gender],
FORMAT(SUM(PurchaseAmount), 'c0') AS [Total Purchase Amount],
FORMAT(Avg(PurchaseAmount),'c0') as [Average purchase amount],
Count(B.BorrowerID) as [No of borrowers],
Floor(Avg(Floor((Datediff(Day,DoB,Getdate()))/365.25 ))) AS [AVERAGE AGE OF BORROWERS],
Format(Avg(LTV),'P') as [Ave LTV],
Format(Min(LTV),'P') AS [Min LTV],
Format(Max(LTV),'P') AS [Max LTV]
FROM [dbo].[Borrower] AS A
